...
export default function Estatisticas() {
  ...
  const irmaosPorFuncao = {
    microfone: irmaos
      .filter((i) => i.ativo && i.funcoes.includes("microfone"))
      .map((i) => i.nome),
    indicador: irmaos
      .filter((i) => i.ativo && i.funcoes.includes("indicador"))
      .map((i) => i.nome),
    audioVideo: irmaos
      .filter((i) => i.ativo && i.funcoes.includes("audioVideo"))
      .map((i) => i.nome),
    estacionamento: irmaos
      .filter((i) => i.ativo && i.funcoes.includes("estacionamento"))
      .map((i) => i.nome),
  };
      ...
      <div className="estatisticas-grid">
        <div className="estatistica-card">
          ...
          <p className="card-info">
            {irmaosPorFuncao.microfone.length} irmaos ativos
          </p>
        </div>
        ...
        <div className="estatistica-card">
          ...
          <p className="card-info">
            {irmaosPorFuncao.indicador.length} irmaos ativos
          </p>
        </div>
        ...
        <div className="estatistica-card">
          ...
          <p className="card-info">
            {irmaosPorFuncao.audioVideo.length} irmaos ativos
          </p>
        </div>

        <div className="estatistica-card">
          <h3 className="card-titulo">Estacionamento</h3>
          <p className="card-info">
            {irmaosPorFuncao.estacionamento.length} irmaos ativos
          </p>
        </div>
      </div>
    </div>
